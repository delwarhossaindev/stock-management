...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4>{{ __('Customers') }}</h4>
    <div>
        <a href="{{ route('customers.export') }}" class="btn btn-success"><i class="bi bi-download"></i> {{ __('Export') }}</a>
        @if(auth()->user()->isAdmin())
        <button class="btn btn-info" data-bs-toggle="modal" data-bs-target="#importModal"><i class="bi bi-upload"></i> {{ __('Import') }}</button>
        <a href="{{ route('customers.create') }}" class="btn btn-primary"><i class="bi bi-plus"></i> {{ __('New Customer') }}</a>
        @endif
    </div>
</div>
<div class="card">
            ...
            <form action="{{ route('customers.import') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="text-muted small">
                        ...
                        <a href="{{ route('customers.sample') }}">{{ __('Download sample file') }}</a>
                    </div>
                </div>
